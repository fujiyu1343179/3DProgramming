...
void Character::Update()
{
	...
	if (spCamera)
	{
		moveVec = moveVec.TransformNormal(moveVec, spCamera->GetRotationMatrix());
	}
	...
	// キャラクターのワールド行列を創る処理
	Math::Matrix _rotation = Math::Matrix::CreateRotationY(DirectX::XMConvertToDegrees(m_worldRot.y));
...
{
	//何も入力がない場合は処理しない
	if (srcMoveVec.LengthSquared() == 0.f) { return; }// LengthSquared：長さがあるかどうか

	//キャラの「正面方向」のベクトル
	Math::Vector3 _nowDir = GetMatrix().Backward();// Backward：矢印のベクトル値を返す(Forward())
	_nowDir.Normalize();
