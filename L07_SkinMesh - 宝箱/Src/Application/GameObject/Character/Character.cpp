...
void Character::Init()//スキンメッシュ：頂点が変形するメッシュ(宝箱も)
{
	if (!m_spModel)
	{
		m_spModel = std::make_shared<KdModelWork>();
		//m_spModel->SetModelData("Asset/Models/SkinMeshMan/SkinMeshMan.gltf");
		m_spModel->SetModelData("Asset/Models/Box/TreasureBox.gltf");
...
void Character::Update()
{
	...
	//お試し
	if (GetAsyncKeyState('F')) { m_move = true; }
	if (GetAsyncKeyState('J')) { m_move = false; }
